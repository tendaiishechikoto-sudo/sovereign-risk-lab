"""
Shared HTTP helper - identical convention to phase one's scripts/http_utils.py.
Every ingestion script goes through this so that a network failure or an
unexpected response shape raises loudly instead of silently defaulting to
zero/empty data (project non-negotiable: no silent failure in data loading).
"""
