...
- **The live Zimbabwe FX snapshot (ZimRate) can be unreachable from cloud CI for the same reason as the IMF cross-check above** — the same access-control pattern (reachable from a normal network path, blocked from GitHub Actions runner IPs) has been observed against `zimrate.com`. Unlike the IMF series, this one *is* a real CSRI input (`fx_spread`, 20% weight) and drives the dashboard's dedicated Zimbabwe FX panel, so it can't just be dropped silently. Instead, a failed fetch is handled with the exact same pattern already used for Kenya/Malawi's missing parallel-market data: `raw_components["ZWE"]["fx_spread_pct"]` is set to `null`, an explicit reason is recorded in `components_excluded_reasons`, the CSRI weights are renormalized over the remaining components for that run, `zwe_fx_detail` in `latest_snapshot.json` becomes `{"available": false, "reason": "..."}`, the gap is surfaced in `meta.json`'s `sanity_check_warnings`, and the frontend's FX panel renders an explicit "unavailable this run" message instead of crashing on missing fields or showing a stale number.
...
24 tests, all offline: calculation-logic tests (normalization, index math, exclusion handling — including a regression test for a real bug caught during development, where a blank CSV cell serialized as an invalid bare `NaN` in the JSON output and broke `JSON.parse` in the browser even though Python's own `json` module tolerated it; and a regression test for the ZimRate cloud-CI outage described above, asserting a failed fetch degrades to an excluded, renormalized component rather than crashing or fabricating a value) and ingestion-parser tests against fixture JSON, one of which is a real captured ZimRate API response.
